...
for jsdindex,jsdrow in dfJSD.iterrows(): 
		thisjs=jsdrow['JOBSTREAM']
		if ((thisjs=='BIR_START_DAILY') or (thisjs=='BIR_SET_RUNDATE')):
			rhjs=jsdrow['SUCCESSOR_JOBSTREAM']
			dfJSD.at[jsdindex,'JOBSTREAM']=rhjs
			dfJSD.at[jsdindex,'SUCCESSOR_JOBSTREAM']='xxx'
print('dealt with bir')
print('deal with rows in jsd where js is not in our master list')
for jsdindex,jsdrow in dfJSD.iterrows():
...
while True:
	...
	for jsindex, jsrow in dfJS.iterrows():              #for each entry in list1
		...
		if (foundOnRight==0):
			print('outputing this js next due on left',thisjs) #output it
